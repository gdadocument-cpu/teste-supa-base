  ...
  try {
    ...
    switch (action) {
      ...
      case "recupererAdministration": {
        ...
        const membresParNom = new Map((members ?? []).map((item: any) => [normalise(item.matricule), item]))
        ...
        return json({
          ...
          utilisateurs: profilsAffichables.map((item: any) => ({
            id: item.id, nom: item.display_name, grade: delayedById.get(item.member_id)?.grade ?? memberById.get(item.member_id)?.grade ?? membresParNom.get(normalise(item.display_name))?.grade ?? "Grade non renseigné",
